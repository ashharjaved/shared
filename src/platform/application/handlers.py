...
async def handle_set_config(session: AsyncSession, tenant_id: UUID, key: str, value: Any) -> Tuple[str, Any]:
...
async def get_config(session: AsyncSession, tenant_id: UUID, key: str) -> Tuple[str, Any] | None:
    cache = get_cache(settings.CONFIG_TTL_SECONDS)
...
async def list_configs(session: AsyncSession, tenant_id: UUID, limit: int, offset: int) -> List[Tuple[str, Any]]:
    repo = ConfigRepository(session)
