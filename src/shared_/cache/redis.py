...
from src.shared_.exceptions import DomainConflictError, RateLimitedError
from src.shared_.utils.strings import slugify
from src.config import settings
from src.shared_.errors import DomainError, ErrorCode, ExternalError
...
class RedisClient:
    ...
    def __init__(self, url: Optional[str] = None, namespace: Optional[str] = None):
        # Prefer explicit arg; else take from settings without coercing None -> "None"
        redis_url = url if url is not None else settings.REDIS_URL
        if redis_url is None or not str(redis_url).strip():
            # Fail fast with a clear message so we don’t pass "None" to redis.from_url
            raise ExternalError("REDIS_URL is not set. Provide a valid redis:// or rediss:// URL.")
        self._url = str(redis_url).strip()

        # isolate by environment/app (e.g., "dev:raydian")
        # NOTE: Settings uses 'environment' (not 'env')
    ...
    def __getattr__(self, name: str) -> Any:
        # Delegate missing attrs to underlying client (after connect)
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            self.redis = None
            raise ExternalError(f"Redis connection failed: {e}") from e
    ...
    async def _guard(self, fn: Callable[[], Any]) -> Any:
        ...
        if self.redis is None:
            raise DomainError("Redis client not connected", code=ErrorCode.EXTERNAL_ERROR)
        try:
            return await fn()
        except RedisError as e:
            raise DomainError(f"Redis error: {e}",code=ErrorCode.EXTERNAL_ERROR) from e
    ...
    async def check_rate_limits(
        ...
        *,
        ...
        """
        Increment-and-check rate limits atomically.
        Pass explicit limits for clarity; if a limit is None/0, it's ignored.
        Raises RateLimitedError / DomainConflictError when exceeded.
        """
        await self.ensure_connected()
        now = datetime.utcnow()

        if per_second_limit and per_second_limit > 0:
            # key: <ns>:rl:<tenant>:<channel>:<unix_second>
            sec_key = self._k("rl", channel_id, int(now.timestamp()))
            count = await self._guard(lambda: self.redis.incr(sec_key))  # type: ignore[union-attr]
            if count == 1:
                await self._guard(lambda: self.redis.expire(sec_key, 1))  # type: ignore[union-attr]
            if int(count) > per_second_limit:
                raise RateLimitedError("messages_per_second_limit_exceeded")

        if monthly_limit and monthly_limit > 0:
            ym = f"{now.year:04d}{now.month:02d}"
            # key: <ns>:usage:<tenant>:<channel>:<yyyymm>
            usage_key = self._k("usage", channel_id, ym)
            current = await self._guard(lambda: self.redis.get(usage_key))  # type: ignore[union-attr]
            current_i = int(current) if current is not None else 0
            if current_i >= monthly_limit:
                raise DomainConflictError("monthly_message_limit_exceeded")

    async def _bump_monthly_usage(self, *, channel_id: UUID) -> None:
        """Increase monthly usage counter for reporting/quotas."""
        await self.ensure_connected()
        now = datetime.utcnow()
        ym = f"{now.year:04d}{now.month:02d}"
        usage_key = self._k("usage", channel_id, ym)

        v = await self.incr(usage_key)                     # <-- wrapper, not self.redis.incr
        if int(v) == 1:
            await self.expire(usage_key, 40 * 24 * 3600)       # <-- wrapper, not self.redis.expire
