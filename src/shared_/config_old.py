...
class Settings:
    ...
    # WhatsApp / Provider
    wa_api_base_url: str = "https://graph.facebook.com/v23.0"
    WHATSAPP_APP_SECRET: Optional[str] = None  # for X-Hub-Signature verification
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "environment",
            _validate_choice(self.environment, choices=("dev", "staging", "prod"), key="ENVIRONMENT"),
        )
        object.__setattr__(
            self, "JWT_ALGORITHM",
        ...
        if self.WHATSAPP_APP_SECRET is not None and len(self.WHATSAPP_APP_SECRET) < 8:
            raise ValueError("WHATSAPP_APP_SECRET looks too short")
    ...
    def safe_dict(self) -> dict:
        return {
            ...
            "JWT_SECRET": _mask_secret(self.JWT_SECRET),
            ...
            "WHATSAPP_APP_SECRET": _mask_secret(self.WHATSAPP_APP_SECRET),
...
def get_settings() -> Settings:
    ...
    # Map environment to .env file in the env folder
    env_file_map = {
        "dev": "env/.env.dev",
        "staging": "env/.env.staging",
        "prod": "env/.env.prod",
    }
    env_file_path = env_file_map.get(env, "env/.env.dev")
    #env_file = Path(__file__).resolve().parent.parent / env_file_path
    env_file = Path(__file__).resolve().parent.parent / "env" / f".env.{env}"
    _maybe_load_dotenv(env_file)


    settings = Settings(
        environment=cast(EnvName, _get_env_str("ENVIRONMENT", "dev") or "dev"),
        debug=_get_env_bool("DEBUG", False),
        is_testing=_get_env_bool("IS_TESTING", False),
        DATABASE_URL=_get_env_str("DATABASE_URL",default="dev", required=True) or "",
        redis_url=_get_env_str("REDIS_URL", None),
        secret_key=_get_env_str("SECRET_KEY", required=True,default="dev_secret_change_me") or "",
        JWT_SECRET=_get_env_str("JWT_SECRET", default="change_this_to_a_long_random_string") or "change_this_to_a_long_random_string",
        ...
        wa_api_base_url=_get_env_str("WA_API_BASE_URL", "https://graph.facebook.com/v23.0") or "https://graph.facebook.com/v23.0",
        WHATSAPP_APP_SECRET=_get_env_str("WHATSAPP_APP_SECRET", None),
